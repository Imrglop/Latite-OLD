...
uintptr_t LocalPlayer::getPtr()
{
	auto ptr = GET_LOCALPLAYER();
	return ptr;
}

template <typename T>
T LocalPlayer::push(uintptr_t offset) {
	auto addy = getPtr() + offset;
	T retVal;
	ReadProcessMemory(getHProcess(), (void*)addy, &retVal, sizeof(retVal), NULL);
	return retVal;
}
...
{
	auto ign = add(0x888);
	return memory::ReadVarString(ign);
}
...
bool LocalPlayer::isInGame()
{
	...
	if (GET_LOCALPLAYER() == 0) return false;
	return true;
}
